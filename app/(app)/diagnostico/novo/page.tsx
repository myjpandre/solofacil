...
import { DadosSolo, Propriedade, Cultura } from '@/types';
...
export default function NovaAnalisePage() {
  ...
  async function gerarDiagnostico() {
    ...
    try {
      ...
      setErro(e.message ?? 'Não foi possível gerar o diagnóstico.');
      setEnviando(false);
    }
  }
      ...
      {etapa === 1 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
          ...
          {modoPropriedade === 'existente' &&
            ...
            propriedadesExistentes.length > 0 && (
          ...
          {modoPropriedade === 'nova' && (
            <>
              ...
              <div className="grid grid-cols-2 gap-5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNovaPropriedade((p) => ({
                        ...p,
                        cultura: e.target.value as Cultura,
                      }))
                    }
                    className="input"
                  >
                ...
                <Campo label="Produtividade esperada (kg/ha)">
                  <input
                    type="number"
                    min={100}
                    step={100}
      ...
      {etapa === 2 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-5">
            Informe os valores do laudo de análise de solo. Campos vazios ficam como “não informado”.
          </p>
          <div className="grid grid-cols-2 gap-5">
...
function StepPill({
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className={`text-sm font-medium ${ativa || concluida ? 'text-slate-800' : 'text-slate-400'}`}>
        {titulo}
      </span>
    </div>
  );
