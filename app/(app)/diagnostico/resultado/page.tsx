...
function ResultadoConteudo() {
  ...
  async function exportarPDF() {
    ...
    try {
      ...
      doc.text(`Gerado em: ${formatarData(gerado_em)}`, 14, 39);
      ...
      const aviso = doc.splitTextToSize(
        `As recomendações são estimativas de um motor de regras simplificado com base agronômica regional para o Ceará/Nordeste. Devem ser validadas por profissional habilitado e ajustadas às condições locais.`,
        180
      );
      ...
      {resultado && (
        <>
          <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
            <div>
              <div className="text-xs font-semibold text-green-700 bg-green-50 inline-block px-2.5 py-1 rounded-full mb-2">
                {resultado.propriedade.municipio}
              </div>
              <h1 className="text-2xl font-bold text-slate-800">{resultado.propriedade.nome}</h1>
              <p className="text-sm text-slate-500 mt-1">
                {resultado.propriedade.area_ha} ha · Gerado em {formatarData(resultado.gerado_em)}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-6 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-bold text-slate-800">{clima.temperatura_atual}°C</span>
              ...
              <div className="flex gap-6 text-sm">
                <div>
                  ...
                  <div className="font-semibold text-slate-700">{clima.sensacao_termica}°C</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-semibold text-slate-700">{clima.chance_chuva_hoje}%</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">Chuva prevista</div>
                  <div className="font-semibold text-slate-700">{clima.precipitacao_prevista_mm} mm</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-6">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {resultado.diagnostico.interpretacoes.map((item, idx) => {
                ...
                return (
                  <div key={idx} className={`bg-white rounded-2xl border ${cores.border} p-5`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-semibold text-slate-700">{item.parametro}</span>
          ...
          <details className="bg-white rounded-2xl border border-slate-200 mb-6 group">
            ...
            <div className="px-5 pb-5 text-xs text-slate-500 space-y-2 border-t border-slate-100 pt-4">
              <p>
                ...
                V%atual) × CTC ÷ 10, com meta específica de saturação por bases de acordo com a
                cultura do talhão selecionado.
              </p>
              <p>
                <strong className="text-slate-700">Fósforo e Potássio:</strong> dose proporcional
                à classe de teor do solo (baixo/médio/adequado), limitada a um teto de referência
                específico para a cultura.
              </p>
              <p>
                <strong className="text-slate-700">Nitrogênio:</strong> produtividade esperada do
                talhão (t) × fator de fixação de nitrogênio da cultura.
              </p>
              <p className="pt-1 text-slate-400">
                Motor de regras {VERSAO_MOTOR} — valores de ordem de grandeza para um MVP; não
                reproduzem tabela de nenhuma publicação específica e não substituem laudo e
                recomendação de um engenheiro agrônomo.
              </p>
            </div>
          </details>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-sm text-amber-900">
            ...
            regras simplificado com base agronômica regional para o Ceará/Nordeste. Devem ser
            validadas por profissional habilitado e ajustadas às condições locais.
          </div>
        </>
      )}
    </main>
